...
class char:
	...
	def load_file(self, file, prefix=os.path.join('..', '')):
		...
		with io.open(prefix + os.path.join('data', 'char', '') + file + '.char', 'r') as f:
						...
						if curmove == 'g':
							...
							if savestring not in self.images:
								if temp['src'] not in self.images:
									image = pygame.image.load(prefix + os.path.join('data', 'char', folder[0], temp['src']))
									image.convert_alpha()
									self.images[temp['src']] = image

								self.images[savestring] = self.images[temp['src']].subsurface(pygame.Rect((temp['pos_x'], temp['pos_y']), (temp['width'], temp['height'])))
